    ...
    <footer className="border-t border-border">
      <div className="mx-auto grid max-w-7xl gap-10 px-5 py-12 md:grid-cols-[1.2fr_1fr] lg:px-8">
        <div>
          ...
          <p className="max-w-md text-sm leading-6 text-muted-foreground">Open-source humanitarian infrastructure for service, education, evidence-led research, and transparent collaboration.</p>
        </div>
        <div className="grid grid-cols-2 gap-4 text-sm text-muted-foreground sm:grid-cols-3">
          <Link href="/about" className="hover:text-foreground">Governance</Link>
          <Link href="/treasury" className="hover:text-foreground">Methodology</Link>
          ...
          <Link href="/conversations" className="hover:text-foreground">Daily report</Link>
          <span>Read-only · V1</span>
        </div>
      </div>
      <div className="border-t border-border px-5 py-5 text-center text-xs text-muted-foreground">© 2026 Satnam Satoshi Foundation · Humans govern. AI assists.</div>
    </footer>
  );
}
